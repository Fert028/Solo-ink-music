...
// --- file type helpers (extension is derived from the validated type, with a
// filename fallback for browsers that report an empty MIME type) ---
...
const KNOWN_AUDIO_EXT = ['mp3', 'ogg', 'wav', 'flac', 'aac', 'm4a']
const KNOWN_IMAGE_EXT = ['jpg', 'jpeg', 'png', 'webp', 'gif']

// With client uploads the file goes browser -> Blob directly, so the 4.5 MB
// serverless limit is gone. Change these to whatever cap you want.
const MAX_AUDIO_SIZE = 50 * 1024 * 1024 // 50 MB
const MAX_COVER_SIZE = 5 * 1024 * 1024 // 5 MB

function audioExtFor(file: File): string | null {
  if (AUDIO_EXT[file.type]) return AUDIO_EXT[file.type]
  const ext = file.name.split('.').pop()?.toLowerCase()
  return ext && KNOWN_AUDIO_EXT.includes(ext) ? ext : null
}
function coverExtFor(file: File): string | null {
  if (IMAGE_EXT[file.type]) return IMAGE_EXT[file.type]
  const ext = file.name.split('.').pop()?.toLowerCase()
  if (ext === 'jpeg') return 'jpg'
  return ext && KNOWN_IMAGE_EXT.includes(ext) ? ext : null
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (audioFile.size > MAX_AUDIO_SIZE) {
        setError(`Аудиофайл слишком большой (макс. ${MAX_AUDIO_SIZE / 1024 / 1024} МБ)`)
        return
      }

      // Аудио загружается напрямую в Vercel Blob, минуя serverless-функцию,
      // поэтому лимит 4.5 МБ больше не действует.
      const audioBlob = await upload(
        `uploads/${crypto.randomUUID()}.${audioExt}`,
        audioFile,
        { access: 'public', handleUploadUrl: '/api/upload' },
      )

      // Обложка (если выбрана) — тоже напрямую в Blob.
      let coverUrl: string | null = null
      if (coverFile) {
        const coverExt = coverExtFor(coverFile)
        if (!coverExt) {
          setError('Неподдерживаемый формат обложки')
          return
        }
        if (coverFile.size > MAX_COVER_SIZE) {
          setError(`Обложка слишком большая (макс. ${MAX_COVER_SIZE / 1024 / 1024} МБ)`)
          return
        }
        const coverBlob = await upload(
          `covers/${crypto.randomUUID()}.${coverExt}`,
          coverFile,
          { access: 'public', handleUploadUrl: '/api/upload' },
        )
      ...
      // Запись в БД — крошечный JSON, проходит через функцию без проблем.
